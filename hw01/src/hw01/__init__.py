...
def main() -> None:
    ...
    data = Data(rng=np_rng, num_samples=settings.data.num_samples, sigma=settings.data.sigma_noise, bias=settings.data.bias)
    
    log.info("created data")

    model = NNXModel(rngs=nnx.Rngs(params=model_key), num_basis=settings.training.num_basis)
